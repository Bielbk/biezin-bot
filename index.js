...
const vcard = 'BEGIN:VCARD\n' +
    ...
    'FN:Lucas🧙‍♂️\n' +
    'ORG:Dono do Tiringa;\n' +
    'TEL;type=CELL;type=VOICE;waid=556496467358:+55 (64) 9646-7358\n' +
    'END:VCARD'
...
//  tiringa.logger.level = 'trace'
    ...
    tiringa.on('qr', () => {
        console.log(color('👆'), color(' Escanear o código acima para iniciar o Lucas-BOT!'))
    })
    fs.existsSync('./tiringa.json') && tiringa.loadAuthInfo('./tiringa.json')
    tiringa.on('connecting', () => {
        start('2', 'Conectando o Lucas-BOT...')
    })
    tiringa.on('open', () => {
  ...
  tiringa.logger.error('Reconectando...')
    ...
    tiringa.on('chat-update', async(mek) => {
        try {
            ...
            const ownerNumber = [`556496467358@s.whatsapp.net`, `557499896089@s.whatsapp.net`]
            const liaN = [`556496467358@s.whatsapp.net`]
            const ownerNumberB = [`556496467358@s.whatsapp.net`]
	    ...
	    const jid = sender
            ...
            const isOwner =  [`556496467358@s.whatsapp.net`]
            const isOwnerB =  [`556496467358@s.whatsapp.net`]
          ...
          /*      if (testat === "fdp") {
              ...
              if (testat.match('tiringa')) {
                ...
                if (budy.match(`@557499896089`)) {
                  ...
                  if (budy.match(`@556496467358`)) {
                    sn = ["1", "2", "3"]
                    res = sn[Math.floor(Math.random() * sn.length)]
               /*     if (res == "1") {
                        result = fs.readFileSync(`./src/Lucas.mp3`)
              ...
              //   await dappauhuy(dapuhy, MessageType.text, tescuk, `${ucapanFakereply}`)
               ...
               case 'x9':
                ...
                if (mentioned.length !== 0){
                    ...
                    if (quotedMsg.sender.match('556496467358')) return reply(`🤨`)
...
Link: ${i.swipeUpLink}\n\n𝗜𝗚 𝗦𝗧𝗢𝗥𝗬\n`
                 ...
                 case 'Lucas':
             const baiano = `556496467358@s.whatsapp.net`
                    const tame = {
                        text: `@${baiano.split("@")[0]} `,
                        contextInfo: {
                            mentionedJid: [baiano]
...
🔰 ${send.neon}`
                ...
                case 'take':
                    ...
                    googlelink = `https://wa.me/556496467358?text=${prefix}menu`;
                    applelink = `https://wa.me/556496467358?text=${prefix}menu`;
                    json = {
...
e upar seu level
  ═════◄••LEVEL••►═════`*/
                ...
                addFilter(from)
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        await ffmpeg(`./${media}`)
                            ...
                            .on('end', function() {
                                console.log('Finish')
                                exec(`webpmux -set exif ${addMetadata('Tiringa-BOT', 'Lucas Dev')} ${ran} -o ${ran}`, async(error) => {
                ...
                addFilter(from)
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        exec(`ffmpeg -i ${media} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 800:800 ${rano}`, (err) => {
                            fs.unlinkSync(media)
                            exec(`webpmux -set exif ${addMetadata('Tiringa-BOT', 'Lucas Dev')} ${rano} -o ${rano}`, async(error) => {
                ...
                case 'bugreport':
                    ...
                    tiringa.sendMessage('556496467358@s.whatsapp.net', options, text, {
                        quoted: mek
                    })
                ...
                case 'hora':
                    ...
                    //_GIT DO BOT
                case 'git':
                    tiringa.sendMessage(from, `https://github.com/LucasH/Tiringa-BOT`, text, {
                        quoted: mek
                    })
                    addFilter(from)
                    break
                ...
                case 'pão':
                    if (isOwner) return reply('Oi Lucas')
                    else {
...
`
  ...
  *◍₊ิ۪͡୭⇾Total de membros: ${participants.length} membros`
                ...
                case 'nomeninja':
                    addFilter(from)
                    if (args.length < 1) return reply(`Exemplo: ${prefix}nomeninja Lucas`)
                    tiringa.updatePresence(from, Presence.composing)
                    nome = body.slice(10)
                    try {
...
você se registrou, digite ${prefix}menu para listar meus comandos`
                ...
                case 'hartatahta':
                    if (args.length < 1) return reply(`Exemplo: ${prefix}hartatahta El Lucas`)
                ...
                default:
                    ...
                    if (isCmd) {
                        if (!mek.key.fromMe) {
                           ...
                           const baino = `556496467358@s.whatsapp.net`
                          const tamme = {
